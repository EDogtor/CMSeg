...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d):
        ...
        if cfg.backbone == 'sigma_tiny':
            ...
            from .dual_vmamba import vssm_tiny as backbone
            self.backbone = backbone()
        elif cfg.backbone == 'sigma_small':
            logger.info('Using backbone: sigma_small')
            self.channels = [96, 192, 384, 768]
            from .dual_vmamba import vssm_small as backbone
            self.backbone = backbone()
        else:
            logger.info('Using backbone: sigma_base')
            self.channels = [128, 256, 512, 1024]
            from .dual_vmamba import vssm_base as backbone
            self.backbone = backbone()

        self.aux_head = None

        if cfg.decoder == 'MambaDecoder':
            logger.info('Using Mamba Decoder (与CIPA一致: deep_supervision=False)')
            from .MambaDecoder import MambaDecoder
            self.deep_supervision = False  # 与CIPA保持一致，不使用深度监督

...
from .local_vmamba.local_scan import local_scan_bchw,local_reverse,local_scan

import sys
import os
# Add path for selective_scan_cuda_core
current_dir = os.path.dirname(os.path.abspath(__file__))
# Check standard build path
build_path = os.path.join(current_dir, 'selective_scan/build/lib.linux-x86_64-cpython-310')
if os.path.exists(build_path):
    sys.path.append(build_path)
else:
    # Try looking in the root build directory if structure is flattened
    pass
...
if True:
    ...
    def cross_selective_scan(
        ...
        if softmax_version:
            ...
            y = y.transpose(dim0=1, dim1=2).contiguous().view(B, H, W, -1)
            # Ensure out_norm is on the same device as input
            if out_norm is not None:
                out_norm = out_norm.to(y.device)
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.d_conv > 1:
            ...
            # Ensure z is on the same device as y
            z = z.to(y.device)
            if self.softmax_version:
                y = y * z
            else:
...
class Backbone_VSSM(VSSM):
    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        try:
            ...
            # 获取当前模型的输入通道数
            # patch_embed 是 nn.Sequential，第一个元素是 Conv2d
            current_in_channels = self.patch_embed[0].in_channels
            
            # 检查预训练权重的通道数
            # VMamba 权重的 patch_embed 可能命名为 'patch_embed.0.weight' 或 'patch_embed.proj.weight'
            ckpt_weight_key = 'patch_embed.0.weight'
            if ckpt_weight_key not in _ckpt[key]:
                 if 'patch_embed.proj.weight' in _ckpt[key]:
                     ckpt_weight_key = 'patch_embed.proj.weight'

            if ckpt_weight_key in _ckpt[key]:
                pretrained_weight = _ckpt[key][ckpt_weight_key]
                pretrained_in_channels = pretrained_weight.shape[1]
                
                # 如果通道数不匹配，需要转换
                if pretrained_in_channels != current_in_channels:
                    if pretrained_in_channels == 3 and current_in_channels == 1:
                        print(f"Converting 3-channel pretrained weights to 1-channel grayscale...")
                        # 使用 RGB2Gray 加权平均: Grey = 0.299R + 0.587G + 0.114B
                        new_weight = (0.299 * pretrained_weight[:, 0:1, :, :] + 
                                     0.587 * pretrained_weight[:, 1:2, :, :] + 
                                     0.114 * pretrained_weight[:, 2:3, :, :])
                        _ckpt[key][ckpt_weight_key] = new_weight
                        print(f"Successfully converted patch_embed weights: {pretrained_weight.shape} -> {new_weight.shape}")

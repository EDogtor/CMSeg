...
# 新增：损失函数配置
argparser.add_argument('--loss_type', type=str, default='tversky_bce', 
                       choices=['dice', 'ce', 'combined', 'tversky_bce', 'iou', 'tversky', 'focal'],
                       help='loss function type (推荐: tversky_bce)')
argparser.add_argument('--dice_weight', type=float, default=0.7, help='weight of dice loss (for combined)')
argparser.add_argument('--ce_weight', type=float, default=0.3, help='weight of ce loss (for combined)')
argparser.add_argument('--tversky_weight', type=float, default=0.7, help='weight of tversky loss (for tversky_bce)')
argparser.add_argument('--bce_weight', type=float, default=0.3, help='weight of bce loss (for tversky_bce)')
argparser.add_argument('--tversky_alpha', type=float, default=0.7, help='Tversky alpha (FN weight, 越大越关注漏检)')
argparser.add_argument('--tversky_beta', type=float, default=0.3, help='Tversky beta (FP weight, 越大越关注误检)')
argparser.add_argument('--pos_weight', type=float, default=None, 
                       help='positive class weight (已废弃，不再使用)')
...
argparser.add_argument('--model_type', type=str, default='vmamba', choices=['original', 'vmamba'],
                       help='Model architecture to use')
argparser.add_argument('--normalization', type=str, default='vmamba', choices=['standard', 'vmamba'],
                       help='Normalization mode for input images')
argparser.add_argument('--patience', type=int, default=20, help='early stopping patience')
argparser.add_argument('--warmup_epochs', type=int, default=0, help='number of warmup epochs (0 to disable)')
argparser.add_argument('--warmup_lr', type=float, default=1e-6, help='initial learning rate for warmup')
argparser.add_argument('--weight_decay', type=float, default=1e-2, help='weight decay (CIPA default: 1e-2)')
argparser.add_argument('--eps', type=float, default=1e-8, help='Adam eps (CIPA default: 1e-8)')
argparser.add_argument('--amp', action='store_true', help='use mixed precision training (AMP)')
...
def train_epoch(model, train_loader, optimizer, criterion, device, epoch, scheduler=None, ema=None, scaler=None):
    """训练一个epoch
    ...
    Args:
        ...
        device: 设备
        epoch: 当前epoch
        scheduler: 学习率调度器（可选，CIPA风格每个step更新）
        ema: EMA对象（可选）
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        pet = batch['pet'].to(device)  # [B, 1, H, W]
        mask = batch['mask'].to(device)
        
        # VMamba模型期望3通道输入（与CIPA一致）
        # 将单通道图像repeat成3通道以充分利用预训练权重
        ct = ct.repeat(1, 3, 1, 1)  # [B, 1, H, W] -> [B, 3, H, W]
        pet = pet.repeat(1, 3, 1, 1)  # [B, 1, H, W] -> [B, 3, H, W]
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                elif hasattr(criterion, '__class__') and 'CombinedTverskyBCE' in criterion.__class__.__name__:
                    loss, tversky_loss, bce_loss = criterion(output, mask)
                else:
                    loss = criterion(output, mask)
            
            scaler.scale(loss).backward()
            scaler.step(optimizer)  # 内部会调用 optimizer.step()
            scaler.update()
            # 学习率调度器必须在 optimizer.step() 之后调用（修复警告）
...
def main():
    ...
    # Add timestamp to logdir to prevent overwriting
    if args.train:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        args.logdir = os.path.join(args.logdir, timestamp)
        print(f"Training log directory updated to: {args.logdir}")
    ...
    if args.use_gpu and torch.cuda.is_available():
        try:
            ...
            if args.model_type == 'vmamba':
                logging.info("Initializing VMamba model...")
                C = edict()
                config = C
                ...
                if os.path.exists(pretrained_path):
                    ...
                    logging.warning(f"Pretrained model not found at {pretrained_path}, training from scratch")
                ...
                # Double check - move all modules to device
                for module in model.modules():
                    module.to(device)
                # Also ensure all parameters are on device
                for param in model.parameters():
                    param.data = param.data.to(device)
                    if param.grad is not None:
                        param.grad.data = param.grad.data.to(device)
            else:
                logging.info("Initializing Original CoCoSeg (UNet) model...")
            ...
            device = torch.device('cpu')
            if args.model_type == 'vmamba':
                # CPU fallback config setup if failed in GPU block
                if 'config' not in locals():
                    ...
                    C.bn_momentum = 0.1
                    C.num_classes = 1
                model = CoCoSegVMamba(cfg=config, norm_layer=torch.nn.BatchNorm2d).to(device)
            else:
                model = Unet_resize_conv().to(device)
    else:
        device = torch.device('cpu')
    ...
    if args.train:
        ...
        if args.loss_type == 'combined':
            criterion = CombinedSegLoss(
                ...
                pos_weight=None  # 已移除pos_weight
            )
            print(f"  Dice权重: {args.dice_weight}, BCE权重: {args.ce_weight}")
        elif args.loss_type == 'tversky_bce':
            ...
            print(f"  Tversky权重: {args.tversky_weight}, BCE权重: {args.bce_weight}")
            print(f"  Tversky参数: alpha={args.tversky_alpha} (FN权重), beta={args.tversky_beta} (FP权重)")
        else:
            criterion = get_loss_function(args.loss_type)
        criterion = criterion.to(device)
        
        # 优化器 (使用AdamW，参数分组：与CIPA完全一致)
        # CIPA策略：基于模块类型分组，BatchNorm/LayerNorm的weight和bias都不使用weight_decay
        import torch.nn as nn
        params_group_decay = []
        params_group_no_decay = []
        
        for m in model.modules():
            if isinstance(m, nn.Linear):
                if m.weight is not None and m.weight.requires_grad:
                    params_group_decay.append(m.weight)
                if m.bias is not None and m.bias.requires_grad:
                    params_group_no_decay.append(m.bias)
                ...
                # BatchNorm/LayerNorm的weight和bias都不使用weight_decay（与CIPA一致）
                if m.weight is not None and m.weight.requires_grad:
                    params_group_no_decay.append(m.weight)
                if m.bias is not None and m.bias.requires_grad:
                    params_group_no_decay.append(m.bias)
            elif isinstance(m, nn.Parameter):
                if m.requires_grad:
                    params_group_decay.append(m)
        
        # 处理其他未分类的参数（通过named_parameters）
        ...
        for name, param in model.named_parameters():
            if not param.requires_grad:
                continue
            if id(param) not in classified_params:
                # 未分类的参数，根据名称和形状判断
                if len(param.shape) == 1 or name.endswith('.bias'):
                    params_group_no_decay.append(param)
                else:
                    params_group_decay.append(param)
        
        param_groups = [
            {'params': params_group_decay, 'weight_decay': args.weight_decay},
            {'params': params_group_no_decay, 'weight_decay': 0.0}
        ]
        optimizer = torch.optim.AdamW(param_groups, lr=args.lr, betas=(0.9, 0.999), eps=args.eps)
        print(f"优化器参数分组: {len(params_group_decay)}个参数使用weight_decay={args.weight_decay}, "
              f"{len(params_group_no_decay)}个参数使用weight_decay=0.0")
        
        # 混合精度训练（与CIPA一致）
        scaler = torch.cuda.amp.GradScaler() if args.amp and args.use_gpu else None
        if scaler:
        ...
        # Initialize EarlyStopping - 使用Dice系数（越大越好）
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, mode='max', delta=1e-6)
        print(f"早停机制已启用 (Patience: {args.patience}, 基于Dice系数)")
        
        # 学习率调度器（CIPA风格：基于step数，每个batch更新）
        # 计算每个epoch的step数
        num_steps_per_epoch = len(train_loader)
        total_steps = num_steps_per_epoch * args.epoch
        
        # CIPA的学习率调度策略
        warmup_factor = 1e-3  # CIPA默认：起始lr = lr * 1e-3
        end_factor = 1e-6     # CIPA默认：最终lr = lr * 1e-6
        warmup_steps = args.warmup_epochs * num_steps_per_epoch if args.warmup_epochs > 0 else 0
        
        def lr_lambda(step):
            """CIPA风格的学习率调度函数（基于step数）"""
            if warmup_steps > 0 and step <= warmup_steps:
                # Warm-up阶段：从warmup_factor线性增长到1.0
                ...
                if cosine_steps > 0:
                    return ((1 + np.cos(current_step * np.pi / cosine_steps)) / 2) * (1 - end_factor) + end_factor
                else:
                    return end_factor
        
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_lambda)
        print(f"学习率调度器: 基于step数（每个batch更新）")
        print(f"  - 总steps: {total_steps} (每个epoch {num_steps_per_epoch} steps)")
        if warmup_steps > 0:
            print(f"  - Warm-up: {warmup_steps} steps ({args.warmup_epochs} epochs), 起始lr = lr × {warmup_factor}")
        print(f"  - 最终lr = lr × {end_factor}")
        ...
        if args.resume:
            if os.path.exists(args.resume_ckpt):
                ...
                # 与CIPA一致：checkpoint保存的是已完成的epoch，所以从下一个epoch开始
                ...
                # 恢复学习率调度器状态
                # 注意：LambdaLR在恢复时需要根据已完成的step数重新初始化last_epoch
                # 这样可以避免在第一个batch之前调用scheduler.step()的警告
                # checkpoint保存的是已完成的epoch，所以已完成的step数 = (start_epoch - 1) * num_steps_per_epoch
                if start_epoch > 0:
                    completed_steps = (start_epoch - 1) * num_steps_per_epoch
                    # 重新创建scheduler，设置last_epoch为已完成的step数
                    # 这样scheduler会从正确的位置继续，第一个batch完成后会正确更新到completed_steps + 1
                    scheduler = torch.optim.lr_scheduler.LambdaLR(
                        optimizer, 
                        lr_lambda=lr_lambda,
                        last_epoch=completed_steps - 1  # last_epoch是上一个完成的step
                    )
                    print(f"已重新初始化学习率调度器，从step {completed_steps}继续（已完成{start_epoch - 1}个epoch）")
                else:
                    # 如果没有恢复训练，scheduler已经正确初始化（last_epoch=-1）
                    print(f"学习率调度器已初始化（从头开始训练）")
                # 恢复混合精度scaler状态
                if 'scaler' in checkpoint and scaler is not None:
                    scaler.load_state_dict(checkpoint['scaler'])
                    print(f"已恢复混合精度scaler状态")
                # 恢复EMA状态
        ...
        # 训练循环
        print("\n开始训练...")
        if args.loss_type == 'tversky_bce':
            print(f"配置: loss={args.loss_type}, tversky_weight={args.tversky_weight}, bce_weight={args.bce_weight}")
            print(f"      Tversky: alpha={args.tversky_alpha}, beta={args.tversky_beta}")
        else:
        ...
        for epoch in range(start_epoch, args.epoch):
            ...
            if (epoch + 1) % args.val_freq == 0:
                ...
                # Check early stopping - 使用最佳Dice系数（阈值扫描的）
                early_stopping(best_dice, model)
                if early_stopping.early_stop:
                    print(f"Early stopping triggered after {epoch + 1} epochs (Best Dice: {best_dice:.6f})")
                    break
        ...
        if scaler is not None:
            latest_dict['scaler'] = scaler.state_dict()
